/**
 * Reset Template API Route
 * Handles template reset for quick storyboard configurations
 */
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    ErrorHandler.logError(error, 'Reset Template API');
...
async function handleResetTemplate(req: NextApiRequest, res: NextApiResponse) {
  ...
  const validTypes = ['threeView', 'multiGrid', 'styleComparison', 'narrativeProgression'];
  ...
  try {
    const config = await quickStoryboardService.resetTemplate(
      id as string,
      userId,
      templateType as any
    );
