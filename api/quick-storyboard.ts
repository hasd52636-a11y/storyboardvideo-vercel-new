/**
 * Quick Storyboard API Routes
 * Handles quick storyboard configuration management
 */

import type { NextApiRequest, NextApiResponse } from 'next';
import { quickStoryboardService } from '@/services/QuickStoryboardService';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    ErrorHandler.logError(error, 'Quick Storyboard API');
...
async function handleGetConfig(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (id) {
      ...
      const config = await quickStoryboardService.getConfigById(id as string);
      if (!config) {
      ...
      const config = await quickStoryboardService.getConfig(parseInt(userId as string));
      if (!config) {
        // Create default configuration if not exists
        const defaultConfig = await quickStoryboardService.createConfig({
          userId: parseInt(userId as string),
          name: 'Default Configuration',
          description: 'Default quick storyboard configuration',
        });
...
async function handleCreateConfig(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const config = await quickStoryboardService.createConfig({
      userId,
      name,
      description,
      templates,
...
async function handleUpdateConfig(req: NextApiRequest, res: NextApiResponse) {
  const { id } = req.query;
  const { userId, name, description, threeViewTemplate, multiGridTemplate, styleComparisonTemplate, narrativeProgressionTemplate } = req.body;
  ...
  try {
    const config = await quickStoryboardService.updateConfig(id as string, userId, {
      name,
      description,
      threeViewTemplate,
      multiGridTemplate,
